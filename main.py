...
def _select_driver_face(
    ...
    if len(faces) == 1:
        center = faces[0].get_center()
        return faces[0], center

    if last_center is None:
        best = max(faces, key=lambda f: f.get_center()[0])
        return best, best.get_center()

    best = None
    best_dist = float("inf")
    for f in faces:
        cx, cy = f.get_center()
        d = (cx - last_center[0]) ** 2 + (cy - last_center[1]) ** 2
        if d < best_dist:
            best_dist = d
            best = f
...
def _draw_driver_box(frame, face_result):
    if face_result is None:
        return frame
    x_min, y_min, x_max, y_max = face_result.get_bbox()
    cv2.rectangle(frame, (x_min, y_min), (x_max, y_max), _DRIVER_BOX_COLOR, _DRIVER_BOX_THICK)
    return frame
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    with FaceLandmarkExtractor() as landmark_extractor:
        ...
        while cap.isOpened():
            ...
            driver_face, last_driver_center = _select_driver_face(all_faces, last_driver_center)
            frame = _draw_driver_box(frame, driver_face)
